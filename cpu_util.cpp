...
void InterruptHandler::service_interrupt(interrupt_type type) {
    ...
    ctx.IF &= disable;  // Acknowledge interrupt
...
void InterruptHandler::handle_interrupts() {
    ...
    bool joypad_requested = ctx.IF & 0x10;
    bool serial_requested = ctx.IF & 0x08;
    bool timer_requested  = ctx.IF & 0x04;
    bool LCD_requested    = ctx.IF & 0x02;
    bool VBlank_requested = ctx.IF & 0x01;
    bool joypad_enabled = ctx.IE & 0x10;
    bool serial_enabled = ctx.IE & 0x08;
    bool timer_enabled  = ctx.IE & 0x04;
    bool LCD_enabled    = ctx.IE & 0x02;
    bool VBlank_enabled = ctx.IE & 0x01;
